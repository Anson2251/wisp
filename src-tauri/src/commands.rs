use crate::{cache::AstCache, utils::compute_content_hash, db::{Database, Message}};
...
#[tauri::command]
pub async fn save_message(app_handle: AppHandle, id: String, text: String, sender: String) -> Result<(), String> {
    let db = Database::new(&app_handle).map_err(|e| e.to_string())?;
    db.save_message(&id, &text, &sender).map_err(|e| e.to_string())
}
